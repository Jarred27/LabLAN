...
files=['tmp_real_X.txt','tmp_imaj_X.txt','tmp_real_y.txt','tmp_real_y.txt']; % files in /data_files/
...
for fileName=files
	...
	if normalisationFactor~=1
		warning(["normalising ",fileName," by ",normalisationFactor]) % this is normal so consider supressing
	end
end
